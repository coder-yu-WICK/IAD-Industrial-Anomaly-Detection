...
- 融合规则：计算 PatchCore 二值异常区域与 SAM 候选区域的 mIoU；达到阈值后仅在 SAM 轮廓窄带内平滑 PatchCore 等值线，热力图主体保持原值，不新增异常、不覆盖 SAM 区域。`--sam-surrounding-decay` 控制局部边界平滑强度，`--sam-boundary-width` 控制轮廓窄带宽度；未达到阈值则完全保留 PatchCore 原图。可用 `--category foo` 仅推理指定类别。
